<form action="action_page.php">
  <div class="container">
    <h1>DevSecOps Student Learning Project Q4 2022</h1>
    <br>
    <h1>New Student Registration Form</h1>
    <p>Please fill in this form to create an account.</p>
    <hr>
     
    <label for="Name"><b>Enter Name</b></label>
    <input type="text" placeholder="Enter Full Name" name="Name" id="Name" required>
    <br>
    
    <label for="mobile"><b>Enter mobile</b></label>
    <input type="text" placeholder="Enter moible number" name="mobile" id="mobile" required>
    <br>

    <label for="email"><b>Please Enter Email</b></label>
    <input type="text" placeholder="Enter Email" name="email" id="email" required>
    <br>

    <label for="psw"><b>La Contrasena</b></label>
    <input type="password" placeholder="Enter Password" name="psw" id="psw" required>
    <br>

    <label for="psw-repeat"><b>Please Repeat Password</b></label>
    <input type="password" placeholder="Repeat Password" name="psw-repeat" id="psw-repeat" required>
    <hr>
    <br>
    <p>By creating your student account you agree to our <a href="#">Terms & Privacy</a>.</p>
    <button type="submit" class="registerbtn">Register</button>
  </div>
  <div class="container signin">
    <p>Do you already have an account? <a href="#">Sign in</a>.</p>
  </div>

   <h1> Thank you & Happy Learning! ! ! </h1>
   <h1> God Bless You, my friend. </h1>
   <h1> Have a great day & see you next time. </h1> 
   <h1> Ok good bye & hope to see you again very soon my dear friend friend :) </h1>
</form>
